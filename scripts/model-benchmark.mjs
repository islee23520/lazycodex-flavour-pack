...
import { readActiveModelProvider, fetchAvailableModels } from "./model-provider.mjs";
import { BENCHMARK_SCENARIOS, DEFAULT_BENCHMARK_ROLES } from "./model-benchmark-scenarios.mjs";
import { recommendFromAvailableModels } from "./model-benchmark-recommendations.mjs";
import { applyRecommendedOverrides, readCurrentOverrideConfig } from "./model-benchmark-overrides.mjs";
import { runSingleBenchmark, writeBenchmarkResult } from "./model-benchmark-results.mjs";
...
export async function runModelBenchmarks(args, options = {}) {
  ...
  const availableModels = args.models ?? options.models ?? (await fetchAvailableModels({ ...options, fetch: fetchImpl }));
  ...
  if (args.recommendOnly) {
    const recommendations = recommendFromAvailableModels({ roles, models: candidates, currentOverrides: currentConfig.overrides });
  ...
  for (const role of roles) {
    ...
    for (const model of candidates) {
      for (let sample = 0; sample < args.samples; sample += 1) {
        runs.push(await runSingleBenchmark({ role, model, scenario, provider, fetchImpl, env, dryRun: args.dryRun, sample }));
      }
    }
  }
...
function recommendWinners(runs, currentOverrides) {
  ...
  for (const [role, items] of byRole.entries()) {
    ...
    recommendations[role] = {
      model: winner.model,
      model_reasoning_effort: current.model_reasoning_effort ?? BENCHMARK_SCENARIOS[role].currentFallback.model_reasoning_effort,
...
function selectCandidates(models, roles, overrides) {
  ...
  for (const model of models) {
    if (/^(gpt-5\.5|gpt-5\.4-mini|gpt-5\.3-codex-spark|grok-3-mini-fast|grok-4\.20-0309-reasoning|glm-5\.2|glm-5\.1)$/.test(model)) {
      wanted.add(model);
    }
  }
...
function redactProvider(provider) {
  return { id: provider.id, baseUrl: provider.baseUrl, bearerToken: provider.bearerToken ? "<redacted>" : null, bearerTokenEnv: provider.bearerTokenEnv };
}
