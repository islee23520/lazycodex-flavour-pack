...
import { parseSyncArgs } from "../src/cli/cli-args.ts";
...
test("given npx-style CLI setup when upstream agent exists then applies configured model fields", () => {
  ...
  try {
    ...
    assert.equal(
      result.stdout.indexOf("lazycodex-ai install stub") < result.stdout.indexOf("installed lfp@islee23520"),
      true
    );
...
test("given local CLI setup skips LazyCodex install when requested then installs checkout files", () => {
  ...
  try {
    ...
    assert.equal(
      existsSync(path.join(codexHome, "local-marketplaces", "islee23520", "plugins", "lfp", "scripts", "cli.mjs")),
      true
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
...
test("given non-interactive setup without saved overrides then writes recommendations and applies configured agents", async () => {
  ...
  try {
    ...
    writeOmo410AgentFixtureSet(agentsDir, { explorer: { model: "upstream-original" } });
    writeFileSync(
    ...
    const result = await spawnCli(["setup", "--skip-lazycodex-install", "--skip-model-prompt"], {
...
test("given saved LFP overrides when setup skips model prompt then applies configured agents", () => {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [CLI, "setup", "--skip-model-prompt", "--skip-lazycodex-install"], {
    ...
    const installedOverrideText = readFileSync(
      path.join(
        ...
        "lfp",
        "agent-configs",
        "omo-agent-model-overrides.toml"
      ),
      "utf8"
    );
...
test("given saved LFP overrides when dry setup runs then reports no override drift", () => {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [CLI, "dry-setup", "--skip-model-prompt", "--skip-lazycodex-install"], {
      env: { ...process.env, CODEX_HOME: codexHome },
      encoding: "utf8"
    });
...
test("given agent-config explicitly syncs global defaults then virtual sections update Codex defaults", () => {
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [CLI, "agent-config", "--config", fixture.configPath, "--sync-global-defaults"],
      {
        env: { ...process.env, CODEX_HOME: fixture.codexHome, HOME: root },
        input: "\n".repeat(9),
        encoding: "utf8"
...
test("given doctor fix-cache apply sees duplicate Codex Apps cache then quarantines and rechecks healthy", () => {
  ...
  try {
    ...
    assert.match(
      result.stdout,
      new RegExp(`quarantined duplicate Codex Apps tool cache ${escapeRegExp(cachePath)} -> .*_fetch`)
    );
...
test("given doctor apply is passed without fix-cache then rejects the flag combination", () => {
  ...
  try {
    const result = spawnSync(
      process.execPath,
      [CLI, "doctor", "--apply", "--config", path.join(root, "missing.json")],
      {
        env: cliEnv(root),
        encoding: "utf8"
      }
...
function savedOverrideJson(overrides) {
  return `${JSON.stringify({ schemaVersion: 2, source: { agentsDir: "${CODEX_HOME}/agents" }, overrides, rolePolicies: {} }, null, 2)}\n`;
}
