...
import { resolve } from "../src/model/model-fallback-resolver.ts";
import { syncAgentOverrides } from "../src/model/sync-agent-overrides.ts";
  ...
  it("given benchmark applier writes saved upstream overrides and sync applies installed TOML when resolving then keeps primary-only saved JSON", () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        ['name = "explorer"', 'model = "old-primary"', 'model_fallback = "old-installed-fallback"', ""].join("\n")
      );
      writeFileSync(
        configPath,
        [
      ...
      writeFileSync(
        path.join(agentsDir, "explorer.toml"),
        readFileSync(path.join(agentsDir, "explorer.toml"), "utf8").replace(
          'model = "saved-primary"',
          'model = "conflicting-installed-primary"'
        )
      );
