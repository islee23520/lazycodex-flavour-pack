...
test("given LFP-owned agent names when checking then returns empty list (LFP rides on upstream OMO agents)", () => {
...
test("given agent name when checking ownership then returns false for every agent (no LFP-owned agents)", () => {
  ...
  assert.equal(isLfpOwnedAgent("oracle"), false);
  assert.equal(isLfpOwnedAgent("sisyphus-junior"), false);
