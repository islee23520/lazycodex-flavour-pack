...
} from "../src/model/agent-model-config.ts";
import { discoverAdditionalAgents, writeOverrideFields } from "../src/model/agent-model-config-io.ts";
...
test("given recommended agent when user presses enter then shows current versus recommendation without fallback fields", async () => {
  ...
  try {
    ...
    assert.equal(
      configOutput.questions.some((question) => /plan fallback model/.test(question)),
      false
    );
    assert.doesNotMatch(updated, /^model_fallback/m);
  } finally {
...
test("given default and ULW model sections when configuring setup then prompts and writes them before per-agent overrides", async () => {
  ...
  try {
    ...
    assert.ok(
      configOutput.questions.findIndex((question) => /Default Codex model/.test(question)) <
...
test("given discovered LazyCodex code reviewer agent has recommendation when user opts in then writes primary fields only", async () => {
  ...
  try {
    ...
    writeFileSync(configPath, ["[source]", `agents_dir = "${root}"`, ""].join("\n"));
    const output = captureOutput();
    ...
    assert.equal(
      configOutput.questions.some((question) => /lazycodex-code-reviewer fallback model/.test(question)),
      false
    );
...
test("given noninteractive OMO override setup when called then keeps configured model", async () => {
  ...
  try {
    ...
    writeFileSync(
      configPath,
      ["[source]", `agents_dir = "${root}"`, "", "[agents.explorer]", 'model = "grok-4.3"', ""].join("\n")
    );
