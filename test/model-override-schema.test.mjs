...
import {
  ...
  AGENT_NAME_PATTERN
} from "../scripts/model-override-schema.mjs";
...
test("given invalid agent names when parsing then rejects with clear error", () => {
  ...
  for (const name of invalidNames) {
    ...
    assert.throws(() => parseModelOverrideConfig(bad), new RegExp(`Invalid agent name "${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"`));
    assert.throws(() => parseSavedUserModelOverrideConfig({ schemaVersion: 1, overrides: { [name]: { model: "gpt-5" } } }), new RegExp(`Invalid agent name "${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"`));
  }
});
