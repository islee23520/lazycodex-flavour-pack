...
test("given setup model prompt when installed LazyCodex agent exists then shows vanilla recommendation before selection", async () => {
  ...
  try {
    ...
    writeFileSync(path.join(root, "explorer.toml"), agentText("explorer", "vanilla-explorer-model", "medium", "default"));
    writeFileSync(
      configPath,
      [
        "[source]",
...
function agentText(name, model, reasoning, tier) {
  return [`name = "${name}"`, `model = "${model}"`, `model_reasoning_effort = "${reasoning}"`, `service_tier = "${tier}"`, ""].join("\n");
}
