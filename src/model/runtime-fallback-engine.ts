...
let cachedConfig: RuntimeFallbackConfig | null = null;

function loadConfig(): RuntimeFallbackConfig | null {
...
export function getRuntimeFallbackConfig(): RuntimeFallbackConfig | null {
  return loadConfig();
}
...
export function getRetryGuidance(agentName: string, statusCode: number): { emit: boolean; guidance: string | null } {
  const config = loadConfig();
  if (!config) return { emit: false, guidance: null };
  if (!config.retry_on_errors.includes(statusCode)) return { emit: false, guidance: null };
