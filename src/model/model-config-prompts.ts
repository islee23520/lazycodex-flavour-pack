...
export async function promptForServiceTier(
  ...
  printChoices(
    SERVICE_TIERS.map((tier) => formatChoiceWithVanilla(tier.label, tier.value, vanillaRecommendation)),
    output
  );
...
export async function promptForReasoningEffort(
  ...
  printChoices(
    REASONING_EFFORTS.map((effort) => formatChoiceWithVanilla(effort, effort, vanillaRecommendation)),
    output
  );
