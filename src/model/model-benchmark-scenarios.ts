export const BENCHMARK_SCENARIOS = {
  explorer: {
    prompt:
      'Given this repo map: scripts/cli.mjs is the CLI entrypoint, src/model/sync-agent-overrides.ts syncs agent TOML files. Return compact JSON with {"path":"<entrypoint>","reason":"<why>"}.',
    checks: ["scripts/cli.mjs", "entrypoint"],
    currentFallback: { model_reasoning_effort: "low", service_tier: "default" }
  },
  librarian: {
    prompt:
      'You are checking Node ESM CLI docs from memory. Return compact JSON with {"runtime":"Node ESM","test_runner":"node:test","warning":"cite docs when external claims matter"}.',
    checks: ["Node ESM", "node:test", "cite"],
    currentFallback: { model_reasoning_effort: "medium", service_tier: "default" }
  },
  metis: {
    prompt:
      'Audit this config risk: model="glm-5.2", model_reasoning_effort="xhigh" caused an API error because valid levels were low, medium, high. Return compact JSON with {"risk":"<risk>","fix":"<fix>"}.',
  ...
  momus: {
    prompt:
      'Review this flawed plan: "Change all agents to the cheapest model without tests." Return compact JSON with {"verdict":"REJECT","reason":"<missing evidence>"}.',
