...
} from "../model/user-model-overrides.js";
...
export async function runSetupLineMode(args, { check, root, defaultConfig }, options = {}) {
  ...
  if (check) {
    ...
    const installedPath = await installAndMaybePrompt(
      args,
      root,
      configPath,
      installOpenAiCompatProvider,
...
async function installAndMaybePrompt(
  args,
  root,
  configPath,
  installOpenAiCompatProvider,
...
function printPendingSetupActions(pending) {
  ...
  for (const item of appCacheState.duplicateFiles) {
    console.log(
      `would quarantine duplicate Codex Apps tool cache ${item.filePath} (${item.duplicateToolNames.join(", ")})`
    );
  }
}
