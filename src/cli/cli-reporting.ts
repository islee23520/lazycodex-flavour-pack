...
export async function printProviderInventoryVisibility(options = {}) {
  ...
  const families = [...new Set(classifyModelInventory(inventory.models).map((model) => model.family))].sort(
...
export function printAgentModelDrift(configPath, options = {}) {
  ...
  for (const item of drift.items) {
    console.log(
      `lfp ${commandName}: agent model drift: ${item.agentName}: ${item.kinds.join(", ")} (${item.filePath})`
    );
  }
