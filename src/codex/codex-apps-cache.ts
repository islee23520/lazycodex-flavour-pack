...
export function getCodexAppsToolCacheState(options = {}) {
  ...
  for (const filePath of files) {
    ...
    if (duplicateToolNames.length === 0) continue;
...
export function quarantineDuplicateCodexAppsToolCaches(options = {}) {
  ...
  if (state.duplicateFiles.length === 0) return { state, quarantined: [] };
...
function findDuplicateToolNames(filePath) {
  ...
  for (const tool of payload.tools) {
    if (typeof tool?.tool_name !== "string" || tool.tool_name.length === 0) continue;
