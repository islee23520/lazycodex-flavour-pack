...
import { escapeRegExp } from "../utils/toml-string-utils.js";
import { ADDITIONAL_AGENT_CONFIGS, getCodexPluginState, MARKETPLACE_ID, PLUGIN_ID, PLUGIN_REF } from "./codex-plugin-install.js";
...
export function getPendingCodexPluginDeleteActions(options = {}) {
    ...
    const actions = [];
    if (state.pluginFilesInstalled || existsSync(state.pluginRoot))
        actions.push(`remove plugin files from ${state.pluginRoot}`);
    ...
    if (state.pluginEnabled)
        actions.push(`remove plugin ${PLUGIN_REF} from ${state.configPath}`);
...
function removeCodexConfig(state) {
    ...
    const next = hasOtherMarketplacePlugins(state, withoutPlugin)
        ? withoutPlugin
        : removeTable(withoutPlugin, `marketplaces.${MARKETPLACE_ID}`);
    writeFileSync(state.configPath, normalizeTrailingNewline(next));
}
function backupPluginRoot(pluginRoot) {
    if (!existsSync(pluginRoot))
        return { pluginRoot, backupRoot: null };
...
function restorePluginRoot(backup) {
    if (backup.backupRoot === null || !existsSync(backup.backupRoot))
        return;
...
function cleanupPluginBackup(backup) {
    if (backup.backupRoot === null)
        return;
    rmSync(backup.backupRoot, { recursive: true, force: true });
}
function removeEmptyMarketplaceRoot(state) {
    if (hasOtherMarketplacePlugins(state, readTextIfExists(state.configPath)))
        return;
    rmSync(state.marketplaceRoot, { recursive: true, force: true });
}
function hasOtherMarketplacePlugins(state, configText) {
    if (hasOtherMarketplacePluginConfig(configText))
        return true;
    const pluginsRoot = path.join(state.marketplaceRoot, "plugins");
    try {
        return readdirSync(pluginsRoot, { withFileTypes: true }).some((entry) => entry.isDirectory() && entry.name !== PLUGIN_ID && !entry.name.startsWith(`${PLUGIN_ID}.delete-bak-`));
    }
    catch (error) {
...
function hasOtherMarketplacePluginConfig(configText) {
    ...
    while (match !== null) {
        ...
        if (pluginRef !== PLUGIN_REF && pluginRef.endsWith(`@${MARKETPLACE_ID}`))
            return true;
