...
export function readXaiMcpConsent(options = {}) {
    ...
    if (parsed.installXaiMcpPlugin === true)
        return true;
    if (parsed.installXaiMcpPlugin === false)
        return false;
    return null;
}
export function saveXaiMcpConsent(installXaiMcpPlugin, options = {}) {
    ...
    writeFileSync(consentPath, `${JSON.stringify({ installXaiMcpPlugin, recordedAt: new Date().toISOString() }, null, 2)}\n`);
    return consentPath;
}
...
export async function maybePromptXaiMcpPlugin(options = {}) {
    if (!shouldPromptXaiMcpPlugin(options))
        return null;
    ...
    try {
        ...
        if (typeof yesNoSelector === "function") {
            answer = await yesNoSelector({
                question: "Install xAI MCP plugin? Adds Grok image generation, TTS, web search, and video tools to Codex. [y/N]: "
            });
        }
        else {
            answer = await new Promise((resolve) => {
                readline.question("Install xAI MCP plugin? Adds Grok image generation, TTS, web search, and video tools to Codex. [y/N]: ", resolve);
            });
        }
        const shouldInstall = answer === true || (typeof answer === "string" && ["y", "yes"].includes(answer.trim().toLowerCase()));
        ...
        if (shouldInstall) {
            ...
            const result = installXaiMcpPlugin(options);
            if (result.success) {
                console.log("lfp setup: xAI MCP plugin installed. Use `codex-xai-oauth login` or set XAI_API_KEY to authenticate.");
            }
            else {
                console.log(`lfp setup: xAI MCP plugin install failed: ${result.error}`);
        ...
        if (!options.readline && !options.yesNoSelector)
            readline.close();
    }
}
