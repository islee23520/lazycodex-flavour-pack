...
export async function maybeInstallGlobalCli(args, packageRoot, options = {}) {
    if (args.skipGlobalCli === true)
        return { installed: false, skipped: true };
    const shouldInstall = args.globalCli === true || (isInteractive(options) && (await promptForGlobalCliInstall(packageRoot, options)));
...
async function promptForGlobalCliInstall(packageRoot, options) {
    const question = `  Install or update the global lfp CLI now? This runs ${formatGlobalCliInstallCommand(packageRoot)} [y/N]: `;
