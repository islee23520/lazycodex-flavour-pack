...
import { getOpenAiCompatProviderState, hasAnyModelProvider, readOpenAiCompatProviderConfig, upsertOpenAiCompatProvider } from "../provider/codex-provider-config.js";
...
export function getCodexPluginState(options = {}) {
    ...
    return {
        ...
        marketplaceConfigured: marketplaceBlock.includes('source_type = "local"') &&
        ...
        additionalAgentsInstalled: ADDITIONAL_AGENT_CONFIGS.every((fileName) => existsSync(path.join(agentsRoot, fileName))),
        additionalAgentFiles: [...ADDITIONAL_AGENT_CONFIGS, ...REMOVED_LFP_AGENT_FILES].map((fileName) => path.join(agentsRoot, fileName)),
...
export function getPendingCodexPluginActions(options = {}) {
    ...
    if (!state.pluginFilesInstalled)
        actions.push(`install plugin files to ${state.pluginRoot}`);
    if (!state.marketplaceConfigured)
        actions.push(`configure marketplace ${MARKETPLACE_ID} in ${state.configPath}`);
    if (!state.pluginEnabled)
        actions.push(`enable plugin ${PLUGIN_REF} in ${state.configPath}`);
...
function installAdditionalAgents(packageRoot, state) {
    if (ADDITIONAL_AGENT_CONFIGS.length === 0)
        return;
...
function writeMarketplaceManifest(state) {
    ...
    writeFileSync(state.marketplaceManifestPath, `${JSON.stringify({
        name: MARKETPLACE_ID,
        plugins: [
            {
                name: PLUGIN_ID,
...
function upsertCodexConfig(state, options = {}) {
    ...
    const orderedPluginConfig = ensureLfpPluginAfterLazyCodex(upsertTable(pluginConfig, `plugins."${PLUGIN_REF}"`, ["enabled = true"]));
    const next = options.installOpenAiCompatProvider === true
...
function ensureLfpPluginAfterLazyCodex(text) {
    ...
    if (lfpMatch === null || lazyCodexEnd === null || lfpMatch.start > lazyCodexEnd)
        return text;
...
function findLastLazyCodexPluginEnd(text) {
    ...
    for (const table of findTopLevelPluginTables(text)) {
        if (isLazyCodexPluginRef(table.pluginRef))
            lastEnd = table.end;
    }
    return lastEnd;
}
...
function upsertTable(text, tableName, lines) {
    ...
    if (pattern.test(text))
        return text.replace(pattern, `$1${block.trimEnd()}`);
