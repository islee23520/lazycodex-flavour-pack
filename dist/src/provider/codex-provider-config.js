...
import { escapeRegExp, getTableBlock, readTomlString, readTopLevelTomlString } from "../utils/toml-string-utils.js";
...
export function upsertOpenAiCompatProvider(text, provider) {
    ...
    if (state.status === "drifted")
        return text;
    const withActiveProvider = state.activeStatus === "missing" ? upsertTopLevelString(text, "model_provider", provider.id) : text;
    ...
    if (provider.envKey)
        lines.push(`env_key = ${JSON.stringify(provider.envKey)}`);
...
function parseOpenAiCompatProviderConfig(text, filePath) {
    ...
    for (const [key, value] of Object.entries(provider)) {
        if (value === null)
            throw new Error(`Provider config ${filePath} is missing ${key}`);
    }
    return provider;
}
function getProviderStatus(providerBlock, provider) {
    if (providerBlock.length === 0)
        return "missing";
    if (readTomlString(providerBlock, "base_url") === provider.baseUrl &&
        ...
        if (provider.envKey && readTomlString(providerBlock, "env_key") !== provider.envKey)
            return "drifted";
        return "configured";
    }
    return "drifted";
...
function upsertTopLevelString(text, key, value) {
    ...
    if (pattern.test(text))
        return text.replace(pattern, `$1${line}`);
