...
import { getProviderModelInventoryState } from "../model/model-provider.js";
import { ROLE_POLICY_REPORT_ORDER, readRolePolicyConfig } from "../model/role-policy-config.js";
import { readTomlString } from "../utils/toml-string-utils.js";
export function printCodexAppsCacheQuarantine() {
    ...
    for (const item of result.quarantined) {
        console.log(`lfp setup: quarantined duplicate Codex Apps tool cache ${item.filePath} -> ${item.targetPath} (${item.duplicateToolNames.join(", ")})`);
    }
    return false;
}
...
export function printCodexAppsCacheFixPreview() {
    ...
    for (const item of state.duplicateFiles) {
        console.log(`lfp doctor: would quarantine duplicate Codex Apps tool cache ${item.filePath} (${item.duplicateToolNames.join(", ")})`);
    }
...
export function printCodexAppsCacheFixApply() {
    const result = quarantineDuplicateCodexAppsToolCaches();
    for (const item of result.quarantined) {
        console.log(`lfp doctor: quarantined duplicate Codex Apps tool cache ${item.filePath} -> ${item.targetPath} (${item.duplicateToolNames.join(", ")})`);
    }
    const state = getCodexAppsToolCacheState();
    if (state.healthy) {
        console.log("lfp doctor: Codex Apps tool cache: ok");
...
export async function printProviderInventoryVisibility(options = {}) {
    ...
    if (inventory.status !== "available") {
        console.log(`lfp ${options.commandName ?? "doctor"}: provider inventory: degraded visibility (${redactSecretText(inventory.error ?? "unavailable")}); keeping current saved/configured values; manual model entry remains available`);
        return { ok: true, inventory };
    }
    const families = [...new Set(classifyModelInventory(inventory.models).map((model) => model.family))].sort((left, right) => left.localeCompare(right));
    console.log(`lfp ${options.commandName ?? "doctor"}: provider inventory: ${inventory.models.length} models (families: ${families.join(", ")})`);
    return { ok: true, inventory };
}
