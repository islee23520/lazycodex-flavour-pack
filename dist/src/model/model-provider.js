...
export async function getProviderModelInventoryState(options = {}) {
    ...
    if (!response.ok)
        return { status: "unavailable", provider, models: [], error: `HTTP ${response.status}` };
    let payload;
    try {
    ...
    if (models.length === 0)
        return { status: "unavailable", provider, models, error: "provider returned no models" };
    return { status: "available", provider, models, error: null };
}
export function readActiveModelProvider(options = {}) {
    const env = options.env ?? process.env;
    const configPath = options.codexConfigPath ??
    ...
    if (activeProvider === null)
        return { id: null, baseUrl: null, bearerToken: null, bearerTokenEnv: null };
...
function readBearerTokenFromEnv(provider, env) {
    if (provider.bearerTokenEnv && env[provider.bearerTokenEnv]?.trim())
        return env[provider.bearerTokenEnv].trim();
