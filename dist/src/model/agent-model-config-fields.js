import { getAgentDescription, getAgentDisplayName } from "./agent-model-metadata.js";
import { logAgentGuide } from "./model-config-prompts.js";
import { formatPrimaryFields, getModelSetupGuide } from "./model-setup-guidance.js";
...
export function getVanillaPrimaryFields(fields) {
    if (fields === null || typeof fields !== "object" || typeof fields.model !== "string")
        return null;
    return {
        model: fields.model,
...
export function logAgentCurrentAndRecommendation(output, agentName, currentFields, recommendation, vanilla) {
    ...
    output?.log?.(`  Original/current: ${current.model ?? "unknown"} (reasoning: ${current.model_reasoning_effort}, tier: ${current.service_tier})`);
