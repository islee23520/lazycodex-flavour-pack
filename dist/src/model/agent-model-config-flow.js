import { getPrimaryFields, getVanillaPrimaryFields, logAgentCurrentAndRecommendation, logSetupGuide, mergePrimary } from "./agent-model-config-fields.js";
import { getAgentDisplayName } from "./agent-model-metadata.js";
import { BACK_SELECTION, logAgentGuide, promptForModel, promptForReasoningEffort, promptForServiceTier, promptForYesNo } from "./model-config-prompts.js";
...
async function runPromptStep(config, step, options) {
    if (step.type === "default")
        return promptForModelSection(config, step.agentName, {
    ...
    if (step.type === "agent")
        return promptForConfiguredAgent(config, step.agentName, {
            models: options.models,
            output: options.output,
...
async function promptForAdditionalAgent(config, agent, options) {
    while (true) {
        const shouldChange = await promptForYesNo(options.readline, `  Change ${agent.name} (current: ${agent.model ?? "unknown"}) model/tier/reasoning too? [y/N]: `, { yesNoSelector: options.yesNoSelector });
        if (shouldChange === BACK_SELECTION)
            return BACK_SELECTION;
        if (!shouldChange)
            return null;
