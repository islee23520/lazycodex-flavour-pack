...
function classifyCapabilities(modelName) {
    const capabilities = [];
    if (/\b(?:mini|fast|flash|lite|nano)\b/i.test(modelName))
        capabilities.push("fast");
    if (/(?:reasoning|thinking|reasoner|\bpro\b|\bopus\b|^gpt[-_.]?[1-9]|^glm[-_.]?[1-9]|^grok[-_.]?4)/i.test(modelName)) {
        capabilities.push("reasoning");
    }
    if (/(?:vision|visual|vl|multimodal)/i.test(modelName))
        capabilities.push("vision");
    if (/(?:image|img|diffusion|sdxl)/i.test(modelName))
