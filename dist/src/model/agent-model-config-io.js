...
import { REMOVED_LFP_AGENT_NAMES } from "./removed-lfp-agents.js";
...
export function discoverAdditionalAgents(sourceDir, overrides) {
    ...
    for (const fileName of safeReadDir(sourceDir)) {
        ...
        if (configured.has(name) || LFP_AGENT_NAMES.has(name) || REMOVED_LFP_AGENT_NAMES.has(name))
            continue;
...
export function readInstalledAgentFields(sourceDir, agentName) {
    if (typeof sourceDir !== "string" || typeof agentName !== "string")
        return null;
    try {
...
export function writeOverrideFields(configPath, overrides) {
    ...
    for (const line of lines) {
        ...
        if (sectionMatch) {
            ...
            if (agentName !== null)
                seenAgentSections.add(agentName);
        }
        ...
        if (agentName !== null && WRITABLE_FIELDS.includes(key)) {
            ...
            if (Object.hasOwn(overrides[agentName] ?? {}, key)) {
                if (overrides[agentName]?.[key] === undefined || overrides[agentName]?.[key] === null)
                    continue;
    ...
    for (const [agentName, fields] of Object.entries(overrides)) {
        if (seenAgentSections.has(agentName))
            continue;
...
function readModelFields(text) {
    const fields = {};
    for (const key of WRITABLE_FIELDS)
        fields[key] = readTomlString(text, key);
    return fields;
}
