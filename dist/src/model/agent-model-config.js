...
import { getUserOverrideConfigPath, hasSavedUserOverrideConfig, migrateLegacyUserOverrideConfig, restoreUserOverrideConfig, saveUserOverrideConfig } from "./user-model-overrides.js";
...
export async function configureAgentModelOverrides(configPath, options = {}) {
    ...
    if (rl === undefined)
        throw new TypeError("readline is required for interactive model override configuration");
    ...
    if (savedOverrideChoice === "keep")
        return readOverrideConfig(configPath, options);
    if (savedOverrideChoice === "adjust")
        options.output?.log?.("Continuing with editable model override prompts.\n");
    ...
    const installedAgentFields = Object.fromEntries(agentNames
    ...
    if (defaultModelNames.length === 0 && agentNames.length === 0 && additionalAgents.length === 0)
        return config;
    ...
    const recommendations = (options.recommendModels === true || options.offerAutoRecommend === true) && models.length > 0
    ...
    if (options.persistUserOverrides !== false)
...
async function maybeRestoreUserOverrideConfig(configPath, userConfigPath, options) {
    ...
    const shouldAdjust = await promptForYesNo(options.readline, `  Adjust LFP model overrides now? Saved settings: ${userConfigPath} [y/N]: `, { yesNoSelector: options.yesNoSelector });
    if (shouldAdjust === BACK_SELECTION)
        return null;
    if (!shouldAdjust) {
