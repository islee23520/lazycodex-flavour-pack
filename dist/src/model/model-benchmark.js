...
export async function runModelBenchmarks(args, options = {}) {
    ...
    if (provider.baseUrl === null)
        throw new Error("No active model provider base_url configured");
...
export function parseBenchmarkArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (item === "--dry-run")
            parsed.dryRun = true;
        else if (item === "--recommend-only")
            parsed.recommendOnly = true;
        else if (item === "--apply")
            parsed.apply = true;
        else if (item === "--roles")
            parsed.roles = splitCsv(readValue(argv, ++index, item));
        else if (item === "--models")
            parsed.models = splitCsv(readValue(argv, ++index, item));
        else if (item === "--samples")
            parsed.samples = parsePositiveInteger(readValue(argv, ++index, item), item);
        else if (item === "--output")
            parsed.outputPath = readValue(argv, ++index, item);
        else if (item === "--config")
            parsed.configPath = readValue(argv, ++index, item);
        else
            throw new Error(`Unknown benchmark option: ${item}`);
    }
...
function selectCandidates(models, roles, overrides) {
    ...
    for (const role of roles)
        if (overrides[role]?.model)
            wanted.add(overrides[role].model);
...
function printBenchmarkSummary(result, output) {
    ...
    for (const [role, recommendation] of Object.entries(result.recommendations)) {
        output.log(`lfp benchmark: ${role}: ${recommendation.model} score=${recommendation.benchmark.avg_score} latency=${recommendation.benchmark.avg_latency_ms}ms${recommendation.changed ? " (candidate change)" : " (current)"}`);
    }
}
