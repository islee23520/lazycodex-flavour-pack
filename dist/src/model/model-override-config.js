...
import { LegacyV1SavedUserOverrideConfigSchema, parseModelOverrideConfig, SavedUserModelOverrideConfigSchema } from "./model-override-schema.js";
...
export function readOverrideConfig(configPath, options = {}) {
    ...
    if (configPath.endsWith(".json"))
        return normalizeOverrideConfig(JSON.parse(text), env);
    if (configPath.endsWith(".toml"))
        return normalizeOverrideConfig(parseOverrideToml(text), env);
...
function parseOverrideToml(text) {
    ...
    for (const rawLine of text.split(/\r?\n/)) {
        ...
        if (!assignmentMatch || section === null)
            continue;
