import { getModelSettingScope } from "./model-setting-scopes.js";
...
export async function promptForServiceTier(rl, { agentName, displayName, current, vanillaRecommendation, vanillaRecommendationFields, recommendationFields, output, tierSelector }) {
    ...
    if (vanillaRecommendation !== undefined)
        output?.log?.(`  Vanilla LazyCodex service tier: ${vanillaRecommendation}`);
    ...
    while (true) {
        ...
        if (answer.length === 0)
            return current;
        const selected = parseListedSelection(answer, SERVICE_TIERS.map((tier) => tier.value));
        if (selected !== null)
            return selected;
...
export async function promptForReasoningEffort(rl, { agentName, displayName, current, vanillaRecommendation, vanillaRecommendationFields, recommendationFields, output, reasoningSelector }) {
    if (typeof reasoningSelector === "function") {
        return await reasoningSelector({
            agentName,
            displayName: displayName ?? agentName,
            current,
            vanillaRecommendation,
    ...
    if (vanillaRecommendation !== undefined)
        output?.log?.(`  Vanilla LazyCodex reasoning effort: ${vanillaRecommendation}`);
...
export function parseListedSelection(answer, values) {
    if (/^[0-9]+$/.test(answer))
        return values[Number(answer) - 1] ?? null;
...
function parseModelSelection(answer, choices) {
    ...
    for (const choice of choices) {
        if (choice.aliases.includes(answer))
            return answer;
