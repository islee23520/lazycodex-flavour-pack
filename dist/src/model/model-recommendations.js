...
export function buildRecommendedModelOverrides(overrides, models, options = {}) {
    ...
    for (const agentName of Object.keys(overrides ?? {})) {
        if (VIRTUAL_OVERRIDE_SECTIONS.has(agentName))
            continue;
...
export function recommendRoleModelFields(agentName, models, options = {}) {
    ...
    if (compatibleReasoning !== null)
        recommendation.model_reasoning_effort = compatibleReasoning;
...
function legacyFieldPolicy(agentName) {
    ...
    if (reasoningAgent)
        reasoning = "high";
    if (XHIGH_REASONING_AGENT_NAMES.has(agentName))
        reasoning = "xhigh";
    return {
        reasoning,
...
function matchesPreference(model, preference) {
    if (preference.id !== undefined && model.id !== preference.id)
        return false;
    if (preference.family !== undefined && model.family !== preference.family)
        return false;
    if (preference.provider !== undefined && model.provider !== preference.provider)
        return false;
    if (preference.capability !== undefined && !model.capabilities.includes(preference.capability))
        return false;
    if (preference.pattern !== undefined && !preference.pattern.test(model.id))
        return false;
    return true;
}
