...
import { syncAgentOverrides, syncGlobalModelDefaults } from "./sync-agent-overrides.js";
import { createRestoredUserOverrideConfig } from "./user-model-overrides.js";
...
export function runOverrideSyncHook(value = {}, options = {}) {
    ...
    if (eventName !== null && !SUPPORTED_EVENTS.has(eventName))
        return "";
...
function isDirectRun() {
    return (process.argv[1] !== undefined &&
        (import.meta.url === pathToFileURL(process.argv[1]).href ||
            path.basename(process.argv[1]) === "sync-agent-overrides-hook.mjs"));
}
