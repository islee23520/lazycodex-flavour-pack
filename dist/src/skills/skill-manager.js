...
export function parseSkillManagerArgs(argv) {
    ...
    if (sawCheck && sawApply)
        throw new SkillManagerUsageError("--check cannot be combined with --apply");
    return parsed;
}
...
export function collectSkillManagerState(options = {}) {
    ...
    for (const duplicate of findDuplicateSkills(inventory))
...
export function createSkillManagerPlan(state) {
    ...
    for (const issue of state.issues) {
        ...
        if (source === undefined || source.location !== "active")
            continue;
        plannedMoves.push({
            reason: issue.reason,
            from: source.path,
...
function readSkillEntry({ entryPath, location, name, rootPath, rootType }) {
    ...
    try {
        ...
        if (frontmatterName === null)
            return { ...base, valid: false, invalidReason: "missing-frontmatter-name" };
...
function findDuplicateSkills(inventory) {
    ...
    for (const [name, paths] of byName.entries()) {
        if (paths.length > 1)
            issues.push({ kind: "duplicate-skill", name, paths });
    }
    return issues;
}
function resolveDisabledTarget(entry) {
    const disabledRoot = entry.rootType === "agents"
...
function printHumanReport(result, mode, output) {
    ...
    for (const move of result.appliedMoves)
        output.log(`lfp skill-manager: moved ${move.from} -> ${move.to}`);
