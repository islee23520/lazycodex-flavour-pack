export { clearXaiAuth, GROK_XAI_AUTH_BASENAME, GROK_XAI_AUTH_FILE_ENV, readCodexHostOidcForXai, readHostOidcForXai, readXaiAuth, resolveCodexHostAuthPath, resolveGrokDedicatedXaiAuthPath, resolveGrokHostAuthPath, resolveXaiAuthPath, writeXaiApiKey, writeXaiOAuth, XAI_AUTH_BASENAME, XAI_AUTH_DIRNAME, XAI_AUTH_FILE_ENV, XAI_OAUTH_CLIENT_ID, XAI_OAUTH_ISSUER, XAI_OAUTH_TOKEN_URL, XaiAuthUsageError } from "./xai-auth-store.js";
import { readHostOidcForXai, readXaiAuth, resolveCodexHostAuthPath, resolveGrokDedicatedXaiAuthPath, resolveGrokHostAuthPath, resolveXaiAuthPath, writeXaiApiKey, writeXaiOAuth, XAI_OAUTH_CLIENT_ID, XAI_OAUTH_TOKEN_URL, XaiAuthUsageError } from "./xai-auth-store.js";
...
export async function refreshXaiOAuthAsync(options = {}) {
    ...
    if (auth === null)
        throw new XaiAuthUsageError("No xAI auth file to refresh. Run: lfp xai auth import-grok");
    if (auth.apiKey !== undefined)
        return { refreshed: false, mode: "api_key", authFile, expiresAt: null };
...
export function writeImportedGrokXaiAuth(targetPath, auth) {
    ...
    if (auth.apiKey !== undefined)
        writeXaiApiKey(targetPath, auth.apiKey);
    else
        writeXaiOAuth(targetPath, auth);
...
export function getXaiAuthStatus(options = {}) {
    ...
    if (dedicated !== null)
        return statusForDedicatedAuth(authFile, dedicated);
    ...
    if (codexOidc !== null)
        return statusForCodexHostAuth(authFile, codexOidc);
    return {
        ok: false,
        mode: "none",
        authFile,
